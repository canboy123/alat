...
def train(sess, loss, x, y, X_train, Y_train, save=False,
          ...
          c_w=None, c_b=None, pr_model_x=None, pr_model_W=None, pr_model_b=None,
    ...
    with sess.as_default():
        ...
        for epoch in xrange(args.nb_epochs):
            ...
            # Generate enough weights and biases for the batch images
            stack_w_test = []
            stack_b_test = []
            for i in range(args.batch_size):
                randomNum = np.random.randint(0, len(c_w))
                stack_w_test.append(c_w["w"+str(randomNum)])
                stack_b_test.append(c_b["b"+str(randomNum)])
            for batch in range(nb_batches):
                # Compute batch start and end indices
                start, end = batch_indices(
                    batch, len(X_train), args.batch_size)

                xtrain = X_train[index_shuf[start:end]]
                ytrain = Y_train[index_shuf[start:end]]

                '''
                # Apply Pixel Redrawn technique here
                newImages = random_color_set.eval(
                    feed_dict={pr_model_W: stack_w,
                               pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                               pr_model_b: np.reshape(stack_b, [args.batch_size, 1, args.numColorOutput])})
                '''

                # for yinyang in range(2):
                if np.random.randint(0, 10) % 2 == 0:
                    xtrain = 1 - xtrain
                # Perform one training step
                feed_dict = {
                    # x: np.reshape(xtrain, [args.batch_size, 28, 28, 1]),
                    y: ytrain,
                    pr_model_W: stack_w_test,
                    pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                    pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])
                }
            ...
            if save and ((epoch+1) % 25 == 0 or (epoch+1) == args.nb_epochs):
...
def model_eval(sess, x, y, predictions, X_test=None, Y_test=None,
               feed=None, args=None, is_adv=None,
                pred2=None, c_w=None, c_b=None, pr_model_x=None,
               random_color_set=None,ae=None,
                pr_model_W=None, pr_model_b=None, pr_model_output=None):
    """
    ...
    with sess.as_default():
        ...
        for batch in range(nb_batches):
            ...
            if is_adv:
                # Case A
                if case == 1:
                    feed_dict = {x: xtrain, y: Y_cur}
                    adv_x = ae.eval(feed_dict=feed_dict)
                    newImages = random_color_set.eval(
                        feed_dict={pr_model_W: stack_w_test,
                                   pr_model_x: np.reshape(adv_x, [args.batch_size, n_input, 1]),
                                   pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])})
                # Case B
                elif case == 2:
                    feed_dict = {x: np.reshape(xtrain, [args.batch_size, 28, 28, 1]), y: Y_cur}
                    adv_x = ae.eval(feed_dict=feed_dict)
                    newImages = adv_x
                # Case C
                elif case == 3:
                    newImages2 = random_color_set.eval(
                        feed_dict={pr_model_W: stack_w_test,
                                   pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                                   pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])})
                    feed_dict = {x: np.reshape(newImages2, [args.batch_size, 28, 28, 1]), y: Y_cur}
                    adv_x = ae.eval(feed_dict=feed_dict)
                    newImages = adv_x
                else:
                    newImages = xtrain
            ...
            feed_dict = {
                # x: np.reshape(newImages, [args.batch_size, 28, 28, 1]),
                y: Y_cur,
                pr_model_W: stack_w_test,
                pr_model_x: np.reshape(newImages, [args.batch_size, n_input, 1]),
                pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])
            }
            ...
            if showImg:
                imgIndex = 0
                for imgIndex in range(len(newImages)):
                    fig = plt.figure()
                    pixels = newImages[imgIndex].reshape((28, 28))
                    sub = fig.add_subplot(2, 2, 1)
                    plt.imshow(pixels, cmap='gray')
                    '''
                    '''
                    pixels = X_cur[imgIndex].reshape((28, 28))
                    sub = fig.add_subplot(2, 2, 2)
                    plt.imshow(pixels, cmap='gray')
                    pixels = adv_x[imgIndex].reshape((28, 28))
                    sub = fig.add_subplot(2, 2, 3)
                    plt.imshow(pixels, cmap='gray')
                    pixels = np.abs(adv_x[imgIndex].reshape((28, 28))-newImages2[imgIndex].reshape((28, 28)))
