    ...
    with sess.as_default():
        ...
        for epoch in xrange(args.nb_epochs):
            ...
            for i in range(args.batch_size):
                stack_w_test.append(c_w)
                stack_b_test.append(c_b)
        ...
        for batch in range(nb_batches):
            ...
            if is_adv:
                ...
                # Case B
                    ...
                    feed_dict = {x: np.reshape(xtrain, [args.batch_size, 32, 32, 1]), y: Y_cur}
                    # adv_x2 = ae.eval(feed_dict=feed_dict)
                    # feed_dict={pr_model_W: stack_w_test,
                    #            pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                    #            pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])}
                    adv_x = ae.eval(feed_dict=feed_dict)
                ...
                # Case C
                    ...
                    newImages2 = random_color_set.eval(
                        feed_dict={pr_model_W: stack_w_test,
                                   pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                                   pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])})
                    feed_dict = {x: np.reshape(newImages2, [args.batch_size, 32, 32, 1]), y: Y_cur}
                    adv_x = ae.eval(feed_dict=feed_dict)
                    newImages = adv_x
                else:
                    newImages = xtrain
            ...
            feed_dict = {
                x: np.reshape(newImages, [args.batch_size, 32, 32, 1]),
                y: Y_cur,
                # pr_model_W: stack_w_test,
            ...
            if showImg:
                imgIndex = 0
                fig = plt.figure()
                pixels = newImages[imgIndex].reshape((32, 32))
                sub = fig.add_subplot(2, 2, 1)
                plt.imshow(pixels, cmap='gray')
                pixels = X_cur[imgIndex].reshape((32, 32))
                sub = fig.add_subplot(2, 2, 2)
                plt.imshow(pixels, cmap='gray')
                pixels = adv_x2[imgIndex].reshape((32, 32))
    ...
    with sess.as_default():
        ...
        for epoch in xrange(args.nb_epochs):
            ...
            for batch in range(nb_batches):
                ...
                # Perform one training step
                feed_dict = {
                    # x: np.reshape(xtrain, [args.batch_size, 32, 32, 1]),
                    y: ytrain,
                    pr_model_W: stack_w_test,
                    pr_model_x: np.reshape(xtrain, [args.batch_size, n_input, 1]),
                    pr_model_b: np.reshape(stack_b_test, [args.batch_size, 1, args.numColorOutput])
